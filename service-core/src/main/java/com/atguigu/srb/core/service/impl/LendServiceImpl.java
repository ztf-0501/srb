...
public class LendServiceImpl extends ServiceImpl<LendMapper, Lend> implements LendService {
    ...
    public void makeLoan(Long lendId) {
        ...
        paramMap.put("agentBillNo",agentBillNo);  //"LOAN" + 根据当前时间+3个10以内的随机数生成;

        //lend.getServiceRate():平台服务费率/12 保留8位
        //月年化
        BigDecimal monthRate = lend.getServiceRate().divide(new BigDecimal(12), 8, BigDecimal.ROUND_DOWN);
        ...
        //给借款账号转入金额
        BigDecimal total = new BigDecimal(result.getString("voteAmt")); //voteAmt:投资人的投资金额
        userAccountMapper.updateAccount(bindCode,total,new BigDecimal(0));  //根据绑定的id查询到用户ID，再根据用户ID修改账户信息
        ...
        TransFlowBO transFlowBO = new TransFlowBO(
                ...
                bindCode,
                total,
                TransTypeEnum.BORROW_BACK,  //放款到账
                "借款放款到账，编号：" + lend.getLendNo()
        );
        transFlowService.saveTransFlow(transFlowBO);

        //(4)解冻并扣除投资人资金
        //获取标的下的投资列表(标的ID：lendId)
        //status；状态（0：默认 1：已支付 2：已还款）
        //标的会有多位投资人所以要根据标的ID和状态查询投资该标的的所有信息
