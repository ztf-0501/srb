...
public class LendItemServiceImpl extends ServiceImpl<LendItemMapper, LendItem> implements LendItemService {
    ...
    public String commitInvest(InvestVO investVO) {
        ...
        BigDecimal expectAmount = lendService.getInterestCount(
                lendItem.getInvestAmount(), //投资金额
                lendItem.getLendYearRate(),  //年化利率
                lend.getPeriod(),  //投资期数
                lend.getReturnMethod());  //还款方式
        ...
        baseMapper.insert(lendItem);//存入lend_item数据库
    ...
    public void notify(Map<String, Object> paramMap) {
        ...
        String voteAmt = (String)paramMap.get("voteAmt"); //冻结金额

        userAccountMapper.updateAccount(
                voteBindCode,
                new BigDecimal("-" + voteAmt),  //传过去负数
                new BigDecimal(voteAmt));

        //修改投资记录的状态(先查询lend_item表中数据)
